...
// Set randomPizzas to .randomPizzaContainer, use randomPizzas in the for loop so computer wont need to access the DOM every time the loop runs
// Got rid of px, less converson equals better results
...
function updatePositions() {
  ...
  var items = document.getElementsByClassName('mover'); //getElementByClassName is faster 
  var phase = document.body.scrollTop / 1250; // moved scrollTop outside of loop to prevent the page from rendering during loop iteration

  for (var i = 0; i < items.length; i++) {
    var movePizza = Math.sin(phase + (i % 5));
    items[i].style.left = items[i].basicLeft + 100 * movePizza + 'px';
  }
